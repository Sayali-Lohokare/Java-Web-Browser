...
/**
 * This package-private class defines one Sayali_browser window for use with 
 ...
 */
class Sayali_BrowserWindow extends Stage {

    private Sayali_WebBrowser Sayali_owner;    // the WebBrowser application
    private WebEngine Sayali_webEngine; // loads and manages pages
    private Menu Sayali_windowMenu;     // holds window-related commands

    /**
     * The constructor sets up the window, and starts loading the
     * initial web page location, if given.  It does not show the window.
     * @param Sayali_browser     the WebBrowser application that is createing this window
     * @param Sayali_initialURL  if non-null, the window will attempt to load this URL
     */
    Sayali_BrowserWindow(Sayali_WebBrowser Sayali_browser, String Sayali_initialURL) {
        
        Sayali_owner = Sayali_browser;
        ...
        Sayali_webEngine = webview.getEngine();
        ...
        Sayali_webEngine.locationProperty().addListener( (o,oldVal,newVal) -> {
        ...
        Sayali_webEngine.titleProperty().addListener( (o,oldVal,newVal) -> {
            if (newVal == null)
                setTitle("Untitled " + Sayali_owner.Sayali_getNextUntitledCount());
            else
                setTitle(newVal); 
        });
        ...
        Sayali_webEngine.getLoadWorker().stateProperty().addListener( (o,oldVal,newVal) -> {
        ...
        cancelButton.setOnAction( e -> { // Cancel the ongoing page load
            if ( Sayali_webEngine.getLoadWorker().getState() == Worker.State.RUNNING )
                Sayali_webEngine.getLoadWorker().cancel();
        ...
        Sayali_webEngine.setOnAlert( evt -> SimpleDialogs.message(evt.getData(), "Alert from web page") );
        Sayali_webEngine.setPromptHandler( promptData -> 
                    SimpleDialogs.prompt(promptData.getMessage(), 
                                              "Query from web page", promptData.getDefaultValue()));
        Sayali_webEngine.setConfirmHandler( str -> 
                    SimpleDialogs.confirm(str, "Confirmation Needed").equals("yes") );
        
        /* If the Sayali_initialURL is not null, load that page! */
        
        if (Sayali_initialURL != null) {
               // load the initial web page, if any
            doLoad(Sayali_initialURL);
        }
    ...
    private MenuBar makeMenuBar() {
        ...
        newWin.setOnAction( e -> Sayali_owner.Sayali_newBrowserWindow(null) );
        ...
        open.setOnAction( e -> {
            ...
            if (url != null && url.trim().length() > 0)
                Sayali_owner.Sayali_newBrowserWindow(url);
        });
        Sayali_windowMenu = new Menu("Window");
        Sayali_windowMenu.getItems().addAll(newWin,close,open,new SeparatorMenuItem());
        Sayali_windowMenu.setOnShowing( e -> populateWindowMenu() );
        MenuBar menubar = new MenuBar(Sayali_windowMenu);
        return menubar;
    }
    ...
    private void populateWindowMenu() {
        ArrayList<Sayali_BrowserWindow> windows = Sayali_owner.getOpenWindowList();
        while (Sayali_windowMenu.getItems().size() > 4) {
            ...
            Sayali_windowMenu.getItems().remove(Sayali_windowMenu.getItems().size() - 1);
        }
        if (windows.size() > 1) {
            ...
            Sayali_windowMenu.getItems().add(item);
            Sayali_windowMenu.getItems().add( new SeparatorMenuItem() );
        }
        for (Sayali_BrowserWindow window : windows) {
            ...
            final Sayali_BrowserWindow win = window; // (for use in a lambda expression)
